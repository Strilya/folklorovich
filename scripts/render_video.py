...
def create_slideshow_video(
    ...
    if reel_type == "visual":
        ...
        return create_superstition_reel(
            ...
            subtitles_russian, subtitles_english
        )
...
def create_superstition_reel(image_paths: list, output_path: str, audio_path: str, music_path: str, srt_russian: str, srt_english: str) -> str:
    """TYPE B: Superstition with clean professional subtitles"""
    
    duration = get_audio_duration(audio_path)
    num_images_needed = int(duration / 2.5) + 1
    
    logger.info(f"Superstition reel: {duration:.1f}s, {num_images_needed} images")
    ...
    # 2.5s per image
    for i, img in enumerate(image_paths):
        inputs.extend(["-loop", "1", "-t", "2.5", "-i", str(img)])
